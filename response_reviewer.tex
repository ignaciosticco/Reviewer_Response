...
% REVISAR ESTE ITEM 
In page 10: The reviewer argues that we cannot refer to $\mathcal{A}$ and $\mathcal{K}$ like we did (with no previous introduction). He (she) proposes that the equations of Appendix A can be introduced there. We followed his suggestion and Eq. 13 and Eq. 14 now appear in here (page 10) to give a more appropriate presentation to the parameters $\mathcal{A}$ and $\mathcal{K}$.\\
...
% PONER EN QUE PARRAFO DE QUE PAGINA ESTA. 
...
% NO ME TERMINA DE CONVENCER ESTE PARRAFO
...
% MENCIONAR EN QUE PARRAFO DE QUE PAGINA SE INTRODUCE. 
